...
import { calcularItem, aplicarDescuentoProrrateado, agregarLineaDelivery } from "./types"
import type {
  BoletaCliente,
  BoletaItem,
  BoletaPayload,
...
export async function registrarYEmitirBoleta(
  ...
  const { data: orden, error: ordenErr } = await supabase
    .from("ordenes_tienda")
    .select("id, items, cliente_snapshot, estado, total, descuento, delivery")
    .eq("id", ordenId)
    .single()
  ...
  // Snapshot común
  const sunatItems = agregarLineaDelivery(
    aplicarDescuentoProrrateado(
      itemsFromCart(items),
      Number(orden.descuento ?? 0)
    ),
    Number(orden.delivery ?? 0)
  )
